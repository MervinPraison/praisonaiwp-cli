...
class TestDbQuery:
    def test_db_query_success(self, runner, mock_db_config, mock_db_ssh, mock_db_wp_client):
        ...
        result = runner.invoke(db_command, ['query', "SELECT * FROM wp_posts WHERE post_status = 'publish'"])
        
        assert result.exit_code == 0
        mock_db_wp_client.db_query.assert_called_once_with("SELECT * FROM wp_posts WHERE post_status = 'publish'")
    
    def test_db_query_empty(self, runner, mock_db_config, mock_db_ssh, mock_db_wp_client):
        """Test database query with no results"""
        mock_db_wp_client.db_query.return_value = []
        
        result = runner.invoke(db_command, ['query', "SELECT * FROM wp_posts WHERE post_title = 'nonexistent'"])
    ...
    def test_db_query_with_server(self, runner, mock_db_config, mock_db_ssh, mock_db_wp_client):
        ...
        result = runner.invoke(db_command, [
            'query', "SELECT COUNT(*) FROM wp_posts", '--server', 'production'
        ])
